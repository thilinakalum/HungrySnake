...
public class SnakeController extends Thread {
  ...
  public SnakeController(Tuple positionDepart) {
    ...
    foodPosition = new Tuple(Main.height - 1, Main.width - 1);
  ...
  private void checkCollision() {
    ...
    for (int i = 0; i <= positions.size() - 2; i++) {
      boolean biteItself =
          posCritique.getX() == positions.get(i).getX() && posCritique.getY() == positions.get(i)
              .getY();
      if (biteItself) {
        stopTheGame();
      }
    ...
    boolean eatingFood =
        posCritique.getX() == foodPosition.y && posCritique.getY() == foodPosition.x;
    if (eatingFood) {
  ...
  private void moveInterne(int dir) {
    switch (dir) {
      case 4:
        headSnakePos.ChangeData(headSnakePos.x, (headSnakePos.y + 1) % 40);
      ...
      case 3:
        if (headSnakePos.y - 1 < 0) {
          ...
          headSnakePos.ChangeData(headSnakePos.x, Math.abs(headSnakePos.y - 1) % 40);
        }
        positions.add(new Tuple(headSnakePos.x, headSnakePos.y));
        break;
      case 2:
        if (headSnakePos.x - 1 < 0) {
          ...
          headSnakePos.ChangeData(Math.abs(headSnakePos.x - 1) % 40, headSnakePos.y);
        }
        positions.add(new Tuple(headSnakePos.x, headSnakePos.y));
